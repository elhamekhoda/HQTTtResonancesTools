...
NLargeJetTtresSTNoSJetSelector::NLargeJetTtresSTNoSJetSelector(const std::string& params) :
        SignValueSelector("NLARGEJETTTRESSTNOSJET_N", params, true) {
    checkValueIsInteger();
...
bool NLargeJetTtresSTNoSJetSelector::apply(const top::Event& event) const {
    ...
    for (const auto* const largeJet : event.m_largeJets) {
        ...
        if (largeJet->pt() > value() &&
                ...
                std::fabs(top::deltaR(*largeJet, *lep)) > 1.5) {
