...
void TtresEventSaverFlatNtuple::initialize(std::shared_ptr<top::TopConfig> config, TFile* file, const std::vector<std::string>& extraBranches) {
   ...
   systematicTree->makeOutputVariable(m_vtxz, "vtxz");
        ...
        systematicTree->makeOutputVariable(m_ljet_bdt_score,   "ljet_bdt_score");
        systematicTree->makeOutputVariable(m_ljet_dnn_score,   "ljet_dnn_score");
...
void TtresEventSaverFlatNtuple::saveEvent(const top::Event& event) {
   ...
   m_mu_z0sig[k] = m_mu_z0[k]/sqrt(mucov(1,1));
    ...
    m_ljet_nghosttrackjet.resize(event.m_largeJets.size(), 0);
    m_ljet_nghosttrackjetb.resize(event.m_largeJets.size(), 0);
    m_ljet_nghosttrackjetbb.resize(event.m_largeJets.size(), 0);
    m_ljet_nghosttrackjetc.resize(event.m_largeJets.size(), 0);
    m_ljet_nghosttrackjetcc.resize(event.m_largeJets.size(), 0);
    m_ljet_nghosttrackjetl.resize(event.m_largeJets.size(), 0);


    m_ljet_good_sub80.resize(event.m_largeJets.size());
    m_ljet_good_sub50.resize(event.m_largeJets.size());
    m_ljet_good_smooth_mt80.resize(event.m_largeJets.size());
    m_ljet_good_smooth_mt50.resize(event.m_largeJets.size());
    m_ljet_good_smooth_ts80.resize(event.m_largeJets.size());
    m_ljet_good_smooth_ts50.resize(event.m_largeJets.size());
    m_ljet_good_smooth_qt80.resize(event.m_largeJets.size());
    m_ljet_good_smooth_qt50.resize(event.m_largeJets.size());
    m_ljet_good_bdt80.resize(event.m_largeJets.size());
    m_ljet_good_dnn80.resize(event.m_largeJets.size());
    m_ljet_good_topo.resize(event.m_largeJets.size());
    m_ljet_bdt_score.resize(event.m_largeJets.size());
    m_ljet_dnn_score.resize(event.m_largeJets.size());
    m_ljet_topo_score.resize(event.m_largeJets.size());



    m_ljet_good.resize(event.m_largeJets.size());
    m_ljet_notgood.resize(event.m_largeJets.size());
    m_ljet_tau32.resize(event.m_largeJets.size());
    m_ljet_tau32_wta.resize(event.m_largeJets.size());
