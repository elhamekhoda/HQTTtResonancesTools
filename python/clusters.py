            ...
            for i in range(nb_try):
                try:
                    ...
                    if not wait_once:
                        ...
                        logger.info(text)
                        logger.debug('fail to do %s function with %s args. %s try on a max of %s (%s waiting time)' %
...
#        fi
        ...
        subprocess.Popen(['chmod','+x',new_prog],cwd=cwd)
    ...
    def get_jobs_identifier(self, path, second_path=None):
        ...
        # target = subprocess.digest(target)[-self.identifier_length:]
...
# check that file exists and is not empty.
    ...
    def get_identifier(self):
        """It in principle provides the variable which retrieves the job id based on the batch system.
           If not defined, create one.
           This is mainly only used for log files naming.
           """
            ...
            try:
                ...
                try:
                    ...
                    if isinstance(exe,str):
                        ...
                        if arg:
                            proc = subprocess.Popen([exe] + arg,  **opt)
                        else:
                            proc = subprocess.Popen(exe,  **opt)
                        pid = proc.pid
                        self.pids.put(pid)
                        proc.wait()
        ...
        try: # to catch KeyBoardInterupt to see which kind of error to display 
            ...
            if self.stoprequest.isSet():
                if isinstance(self.fail_msg, Exception):
                    ...
                    subprocess.sprint(self.fail_msg)
                    raise self.fail_msg[0], self.fail_msg[1], self.fail_msg[2]
...
class SLURMCluster(Cluster):
    ...
    def control(self, me_dir):
        """ control the status of a single job with it's cluster id """
        cmd = "squeue"
        pstatus = subprocess.Popen([cmd], stdout=subprocess.PIPE)
...
class HTCaaSCluster(Cluster):
    ...
    def submit2(self, prog, argument=[], cwd=None, stdout=None, stderr=None,
        ...
        if 'combine' not in prog and 'pythia' not in prog and 'shower' not in prog :
            ...
            a = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.PIPE, cwd=cwd)
...
class HTCaaS2Cluster(Cluster):
    ...
    def metasubmit(self, me_dir=None):
        if self.submitted > 1100 and self.submitted == len(self.submitted_ids): 
            ...
            result1 = subprocess.Popen(command1,  stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.PIPE)  
            result2 = subprocess.Popen(command2,  stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.PIPE)
            ...
            if self.submitted_dirs[0] or self.submitted_exes[0] :
                result = subprocess.Popen(command,  stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.PIPE)
